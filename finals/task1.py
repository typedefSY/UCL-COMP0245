...
def main_1():
    ...
    for lr in learning_rates:
        ...
        for hidden_size in hidden_sizes:
            ...
            ensure_dir(f'images/task1/deep/lr{lr}')
            ...
            plt.legend()
            plt.tight_layout()
            plt.savefig(f'images/task1/deep/lr{lr}/trajectory_hidden{hidden_size}.png')
            plt.close()
    ...
    generate_heatmap(heatmap_data, y_labels=hidden_sizes, x_labels=learning_rates, title="Final Test Loss Heatmap", filename="images/task1/deep/final_test_loss_heatmap.png")
...
def main_2(lr=0.1):
    ...
    for hidden_size in hidden_sizes:
        ...
        model = DeepCorrectorMLP(hidden_size).to(device)
        criterion = nn.MSELoss()
        optimizer = optim.Adam(model.parameters(), lr=lr)
        epochs = 500
        train_losses = []
        ...
        results_str += f'3 Hidden Layers Size: {hidden_size} Final Train Loss: {train_losses[-1]:.6f}, Final Test Loss: {test_losses[-1]:.6f}\n'
        ...
        axes[index].set_title(f'Trajectory with 3 Hidden Layers Sizes {hidden_size}')
    ...
    plt.tight_layout()
    if not os.path.exists(f'images/task1/deeper/lr{lr}'):
        os.makedirs(f'images/task1/deeper/lr{lr}')
    #! Uncomment the following line to save the plot
    plt.savefig(f'images/task1/deeper/lr{lr}/trajectories.png')
    ...
    for hidden_size in hidden_sizes:
        ...
        plt.plot(train_losses, label=f'Train Loss with 3 Hidden Layers Size: {hidden_size}')
    ...
    for hidden_size in hidden_sizes:
        ...
        plt.plot(test_losses, label=f'Test Loss with 3 Hidden Layers Size: {hidden_size}')
    plt.title('Test Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    #! Uncomment the following line to save the plot
    plt.savefig(f'images/task1/deeper/lr{lr}/training_test_loss.png')
    plt.show()
